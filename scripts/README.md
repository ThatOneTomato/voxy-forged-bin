...
> Pure-stdlib Python (`json` + `zipfile`) — no `jq`/`bash`/WSL, so it behaves the
> same on Windows, macOS and Linux. (Replaces the old `validate_mixins.sh`,
> which silently "failed" wherever `jq` wasn't installed.)
...
# After building
./gradlew jar
python3 scripts/validate_mixins.py   # or `python` / `py` on Windows
```
...
task validateMixins(type: Exec) {
    dependsOn jar
    commandLine pythonExe, 'scripts/validate_mixins.py'   // pythonExe resolves python3/python/py
}
