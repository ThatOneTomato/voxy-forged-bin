...
public class IrisVoxyRenderPipeline extends AbstractRenderPipeline {
    ...
    protected int setup(Viewport<?> viewport, int sourceFramebuffer, int srcWidth, int srcHeight) {
        ...
        // In the Iris path the viewport can arrive un-sized (0x0), which would build an incomplete
        // framebuffer and disable LOD under shaders. Fall back to the real window size.
        if (viewport.width <= 0 || viewport.height <= 0) {
            var window = net.minecraft.client.Minecraft.getInstance().getWindow();
            viewport.setScreenSize(window.getWidth(), window.getHeight());
        }
