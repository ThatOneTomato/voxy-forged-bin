        ...
        public static StateEntry deserialize(int id, byte[] data, boolean[] forceResave) {
            try {
                ...
                if (state.isError()) {
                    ...
                    // The stored blockstate failed to parse - typically a modded block whose mod was
                    // updated/removed or whose registry changed since it was ingested. The DataFixerUpper
                    // pass below can itself THROW (e.g. "Unknown type: block_state" when fixing from data
                    // version 0, which predates the typed block_state reference). That exception used to
                    // be uncaught and crashed world-load entirely. These are just regenerable LOD cache
                    // entries, so on ANY fix failure we drop the entry to air instead of crashing.
                    try {
                        ...
                        Logger.error("Blockstate datafix threw, setting to air. id:" + id + " error: " + fixError);
                        forceResave[0] |= true;
