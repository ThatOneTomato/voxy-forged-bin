...
public class WorldConversionFactory {
    ...
    public static VoxelizedSection convert(VoxelizedSection section,
        ...
        return convert(section, stateMapper, blockContainer, biomeContainer, lightSupplier, null);
    }

    /**
     * @param tileOverrides optional per-voxel {@link BlockState} overrides keyed by section-local voxel index
     *                      ({@code (y&lt;&lt;8)|(z&lt;&lt;4)|x}); used to substitute a representative state for blocks that
     *                      have no bakeable model (e.g. LittleTiles tile-blocks). May be {@code null}.
     */
    public static VoxelizedSection convert(VoxelizedSection section,
                                           Mapper stateMapper,
                                           PalettedContainer<BlockState> blockContainer,
                                           PalettedContainerRO<Holder<Biome>> biomeContainer,
                                           ILightingSupplier lightSupplier,
                                           it.unimi.dsi.fastutil.ints.Int2ObjectMap<BlockState> tileOverrides) {
        ...
        if (storage instanceof SimpleBitStorage bStor) {
            ...
            for (int i = 0; i <= 0xFFF; i++) {
                ...
                if (tileOverrides != null) {
                    BlockState ov = tileOverrides.get(i);
                    if (ov != null) bId = stateMapper.getIdForBlockState(ov);
                }
            ...
            if (bId == 0) {//Its air
                ...
                for (int i = 0; i <= 0xFFF; i++) {
                    ...
                    if (tileOverrides != null) {
                        BlockState ov = tileOverrides.get(i);
                        if (ov != null) vId = stateMapper.getIdForBlockState(ov);
                    }
                    byte light = lightSupplier.supply(i&0xF, (i>>8)&0xF, (i>>4)&0xF);
                    data[i] = Mapper.composeMappingId(light, vId, biomes[Integer.compress(i,0b1100_1100_1100)]);
                }
            }
        }
