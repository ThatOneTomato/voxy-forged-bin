...
vec4 computeColour(vec2 texturePos, vec4 colour) {
    ...
    if (doTint) {
        // Normalise the baked tint to full brightness (preserving hue), matching what the Iris LOD
        // shader does. Some modded foliage (e.g. Dynamic Trees leaves) bakes a very dark tint that,
        // multiplied in directly, renders the leaves near-black without shaders; normalising fixes that.
        vec3 t = uint2vec4RGBA(interData.z).yzw;
        float m = max(max(t.r, t.g), t.b);
        // Only lift abnormally dark baked tints (e.g. Dynamic Trees leaves that would render near-black);
        // leave normal foliage tints alone so LOD isn't over-brightened without shaders.
        if (m > 0.004 && m < 0.45) t *= (0.45 / m);
        colour.rgb *= t;
    }
    // No-shader LOD is lit by a flat full lightmap with no smooth AO. A dim multiplier here was previously
    // used to knock it back toward vanilla, but it read too dark; 1.0 makes flat lit LOD surfaces match the
    // vanilla terrain they meet exactly (LOD is then marginally brighter only at concave corners, where it
    // lacks vanilla's AO darkening — acceptable for distant terrain).
...
//This is deprecated, TODO: remove the non mip code path
    ...
    voxy_emitFragment(VoxyFragmentParameters(emitColour, tile, texPos, face, modelId, getLightmap(), tint, model.customId));
