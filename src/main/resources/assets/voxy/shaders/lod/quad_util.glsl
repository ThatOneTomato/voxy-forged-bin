...
void setupQuad(out QuadData quad, const in Quad rawQuad, uvec2 sPos, bool generateAttributes) {
    ...
    // Face indentation (e.g. the ~2/16 water-surface drop, or any non-full-cube face inset) is a real
    // sub-block distance, so it must stay a fixed fraction of ONE block regardless of LOD voxel size.
    // quadStart is later multiplied by lodScale (=2^lodLevel), so pre-divide the indentation by lodScale;
    // otherwise the inset scales with the voxel and the water surface droops by up to 2^lodLevel*2/16 blocks
    // at coarse LOD. At lodLevel 0 (lodScale 1) this is identical to the original.
    float depthOffset = extractFaceIndentation(faceData) / lodScale;
    quadStart += swizzelDataAxis(face>>1, vec3(faceSize.xz, mix(depthOffset, 1.0-depthOffset, float(face&1u))));
